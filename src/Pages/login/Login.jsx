    ...
    return (
        <div className="min-h-screen max-w-[1440px] mx-auto grid grid-cols-2 items-center">
              ...
              <div className="p-5  w-[90%] mx-auto">
                <h3 className='text-4xl font-bold mb-5 text-primary text-center'>Login with contestHub</h3>
                <form onSubmit={handleSubmit(handleLogin)}>
                  <fieldset className="fieldset">
        
        
                    {/* email  */}
                    <label className="label text-base font-semibold text-black">Your Email</label>
                    <input type="email" className="input w-full" placeholder="Email" {...register("email",{required:"Email is required"})} />
                    {errors.email && <p className="text-red-400 font-medium">{errors.email.message}</p>}
        
                    {/* password  */}
                    <label className="label text-base font-semibold text-black">Enter Password</label>
                    <input type="password" className="input w-full" placeholder="Password" {...register("password",{required:"Password is required",pattern:{
                        value:/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[@$!%*#?&]).{6,}$/,
